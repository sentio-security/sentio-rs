...
impl Rule for UncheckedArithmeticRule {
    ...
    fn match_file(&self, file: &ParsedFile, _ctx: &RuleContext<'_>) -> Vec<RuleMatch> {
        let mut collector = ArithmeticCollector { findings: Vec::new() };
...
fn has_field_access(expr: &str) -> bool {
    ...
    if trimmed.chars().all(|c| c.is_ascii_digit() || c == '.' || c == '_' || c.is_ascii_alphabetic() && c.is_ascii_lowercase() && !matches!(c, 'a'..='f'))
        && !trimmed.contains("::")
        && !trimmed.contains('(')
    {
        let without_suffix = trimmed.trim_end_matches(|c: char| c.is_ascii_alphabetic());
        if without_suffix.chars().all(|c| c.is_ascii_digit() || c == '.' || c == '_') {
            return false;
        }
    }
    trimmed.contains('.')
